...
{
    internal class ObtainableSoftwareEntity : InteractableWorldEntity
    {
            ...
            switch (State)
            {
                ...
                case Global.WEStates.IDLE:
                    if (BBox.Intersects(_protag.BBox))
                    {
                        ...
                        {
                            HelperFunctions.UpdateInventory(Global.ItemTypes.SOFTWARE, _itemID, true, SFX.SOFTWARE_TAKEN);
                            if (_flagToSet >= 0)
                            {
